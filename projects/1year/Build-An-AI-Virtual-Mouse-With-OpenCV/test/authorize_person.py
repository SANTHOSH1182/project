    ...
    if landmark_check:  # Checks if landmarks are tracked
        for hand in landmark_check:  # Landmarks for each hand
            for index, landmark in enumerate(hand.landmark):  # Loops through the 21 indexes and outputs their landmark coordinates (x, y, & z)
                draw.draw_landmarks(img, hand,init_hand.HAND_CONNECTIONS)  # Draws each individual index on the hand with connections
...
def fingers(landmarks):
    ...
    for id in range(1, 5):
        if landmarks[tip_ids[id]][2] < landmarks[tip_ids[id] - 3][2]:  # Checks to see if the tip of the finger is higher than the joint
            finger_tips.append(1)
        else:
            finger_tips.append(0)
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if True in matches:
            ...
            if not show_popup:
                ...
                if authorized and automation_start_time and time.time() - automation_start_time > automation_duration:
